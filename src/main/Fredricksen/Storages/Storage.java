    ...
    public void updateFile(TaskList list) {
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(filename))) {
            for (int i = 0; i < list.size(); i++) {
                ...
                /* String dateTime = task.getDateTime() != null
                        ? task.getDateTime().format(DateTimeFormatter.ofPattern("MMM d yyyy, hh:mm a"))
                        : "";
                String toDateTime = task.getToDateTime() != null
                        ? task.getToDateTime().format(DateTimeFormatter.ofPattern("MMM d yyyy, hh:mm a"))
                        : "";
                String fromDateTime = task.getFromDateTime() != null
                        ? task.getFromDateTime().format(DateTimeFormatter.ofPattern("MMM d yyyy, hh:mm a"))
                        : "";
                 */
